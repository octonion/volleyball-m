...
from ncaa._parameter_levels_geocoding
where parameter='year'
);

create table if not exists ncaa._factors_geocoding (
       parameter		text,
       level			text,
       type			text,
       method			text,
...
truncate table ncaa._factors_geocoding;
...
insert into ncaa._factors_geocoding
...
from ncaa._parameter_levels_geocoding npl
left outer join ncaa._basic_factors_geocoding nbf
...
parameter,
avg(raw_factor)
from ncaa._factors_geocoding
where parameter not in ('o_div','d_div')
group by parameter
);

update ncaa._factors_geocoding
